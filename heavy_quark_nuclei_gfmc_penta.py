...
parser.add_argument('--scale_prefactors', type=float, nargs='+', default=None,
                    help='Optional Hylleraas scale factors ηₖ (one per bucket)')

globals().update(vars(parser.parse_args()))

# Will be resized later once the colour buckets are known
if scale_prefactors is None:
    scale_prefactors = []           # <-- empty means “use defaults”
...
if masses == 0.:
    ...
    if N_coord == 2:
        ...
        masses = [1,-1,1,-1]
    elif N_coord == 5:                      # ← pentaquark (Q̄ Q q q q)
        masses = [-1, 1, 1, 1, 1] 
    elif N_coord == 6:
        masses = [1,1,1,1,1,1]
...
# -------------------------------------------------------------------
#  ■  Geometry helpers  (only used by f_R and laplacian_f_R)  ■
# -------------------------------------------------------------------

def cluster_id(i):
    """
    0 → meson (indices 0-1), 1 → baryon (2-3-4)  –– but *only*
    in the singlet pentaquark.  For every other system keep the
    old half-and-half split so legacy results are unchanged.
    """
    if N_coord == 5 and color == "1x1":
        return 0 if i < 2 else 1
    else:
        return 0 if i < N_coord // 2 else 1


# --- bucket-wise colour coefficients for pentaquarks ---------------
colour_coeffs = {
    #             (i,j):  C_ij
    "1x1": { (0,1): -4/3,
             (2,3): -2/3, (2,4): -2/3, (3,4): -2/3 },
    "8x8": { #(0,1): +1/6,
             (0,4): -7/6,
             (0,1): -1/6, (0,3): -1/6,
             (1,2): -7/12, (2,3): -7/12,
...
# ------------------------------------------------------------
#  Colour-bucket enumeration  (one bucket per C_ij in the dict)
# ------------------------------------------------------------
pair_bucket = {p: n for n, p in
               enumerate(sorted(colour_coeffs.get(color, {})))}

nbuckets = max(pair_bucket.values(), default=-1) + 1   # 0 if no buckets


# ------------------------------------------------------------
#  Hylleraas scale factors  η_k   (user may override on CLI)
# ------------------------------------------------------------
# CLI parser gave   scale_prefactors == None   when the flag is absent
if scale_prefactors is None or len(scale_prefactors) == 0:
    # ⇒ use the universal default η_k = 1
    scale_prefactors = [1.0] * nbuckets


# TODO: try removing scale prefacs if still buggy

else:
    # sanity-check: user must give exactly one value per bucket
    if len(scale_prefactors) != nbuckets:
        raise ValueError(
            f"--scale_prefactors expects {nbuckets} numbers for colour '{color}', "
            f"but got {len(scale_prefactors)}: {scale_prefactors}"
        )

# convert to a plain Python list (argparse might give tuple)
scale_prefactors = list(scale_prefactors)
...
pair_a0_host = onp.full((N_coord, N_coord), a0, dtype=float)

if N_coord == 5:                                      # only pentaquark needs it
    # Update ALL pairs, not just those in color coefficients
    for i in range(N_coord):
        for j in range(i+1, N_coord):  # j > i to avoid double-counting
            pair_a0_host[i, j] = pair_a0_host[j, i] = pair_a0(i, j)

# convert to JAX DeviceArray for use inside jit-compiled code
pair_a0_mat = np.asarray(pair_a0_host)

# quick sanity print
for i in range(5):
    for j in range(i+1, 5):
        print((i, j), pair_a0_mat[i, j] / a0)
...
def f_R(Rs, wavefunction=bra_wavefunction, a0=a0, afac=afac,
    ...
    def r_norm_std(pair):
        ...
        return rij_norm * mij                    # *no* a0 division

    def r_norm_penta(pair):
        i, j = pair
        rdiff    = Rs[..., i, :] - Rs[..., j, :]
        rij_norm = np.sqrt(np.sum(rdiff * rdiff, axis=-1))
        mij      = 2 * masses[i] * masses[j] / (masses[i] + masses[j])
        return rij_norm * mij / pair_a0_mat[i, j]

    # choose which helper to use
    r_norm = r_norm_penta if (N_coord == 5 and wavefunction == "product") \
                           else r_norm_std

    # -----------------------------------------------------------
    #  build the exponent
    # -----------------------------------------------------------
    if N_coord == 5 and wavefunction == "product":
        # pair_a0_mat already contains all a₀, afac, same-flavour factors
        r_sum = np.sum(jax.lax.map(r_norm, pairs), axis=0)

    elif N_coord != 5 and wavefunction == "product":
        # legacy tetraquark/dibaryon branch
        r_sum = (np.sum(jax.lax.map(r_norm, product_pairs), axis=0)
                 * (1/a0 - 1/(a0*afac))
                 + np.sum(jax.lax.map(r_norm, pairs), axis=0)/(a0*afac)
                 + np.sum(jax.lax.map(r_norm, same_pairs), axis=0)
                   * (1/(a0*afac*samefac) - 1/(a0*afac)))

    elif N_coord != 5 and wavefunction == "diquark":
...
def laplacian_f_R(Rs, wavefunction=bra_wavefunction,
                  a0=a0, afac=afac, masses=absmasses):
    ...
    # -----------------------------------------------------------
    # 1)  Δ hits ONE exponential  (outer loops over k,l,i,j)
    # -----------------------------------------------------------
                ...
                for i in range(N_coord):
                    for j in range(N_coord):
                        if i != j and j >= i:
                            ...
                            # --- choose the correct pair radius ------------
                            if (N_coord == 5 and wavefunction == "product"):
                                thisa0 = pair_a0_mat[i, j]
                            else:
                                thisa0 = a0
                                if wavefunction == "product":
                            ...
                            if k == i and l == j:
                                nabla_psi *= (
                                    (1/(radial_n*thisa0)**2 - 2/(thisa0*rij))/masses[k]
                                    + (1/(radial_n*thisa0)**2 - 2/(thisa0*rij))/masses[l]
                                ) * hydrogen_wvfn(rij/thisa0, radial_n)
                            else:
                                nabla_psi *= hydrogen_wvfn(rij/thisa0, radial_n)

                nabla_psi_tot += nabla_psi

    # -----------------------------------------------------------
    # 2)  Δ hits TWO different exponentials (gradient–gradient)
    #     The only modification is the same `thisa0` choice.
    # -----------------------------------------------------------
    for a in range(N_coord):
        for k in range(N_coord):
            for l in range(k, N_coord):
                if k != l and (a == k or a == l):
                    for m in range(N_coord):
                        for n in range(m, N_coord):
                            if (m != n and (a == m or a == n)
                                    and (m != k or n != l)):

                                for x in range(3):
                                    nabla_psi = 1.0
                                    for i in range(N_coord):
                                        for j in range(i, N_coord):
                                            if i != j:

                                                # --- again pick the pair radius
                                                if (N_coord == 5 and wavefunction == "product"):
                                                    thisa0 = pair_a0_mat[i, j]
                                                else:
                                                    thisa0 = a0
                                                ...
                                                rsign = (1 if a == i else -1) if (a == i or a == j) else 0
...
# ------------------------------------------------------------------
#  Non-JIT Laplacian (used only when N_coord ≥ 6 and --verbose)
# ------------------------------------------------------------------
if N_coord >= 6 and verbose:
    ...
    def laplacian_f_R(Rs, wavefunction=bra_wavefunction,
        ...
        # ───── first: Laplacian acting twice on the *same* factor ────
        for k in range(N_coord):
            for l in range(k, N_coord):
                if k == l:
                    continue
                ...
                for i in range(N_coord):
                    for j in range(i, N_coord):
                        ...
                        thisa0 = a0
                        if wavefunction == "product":
                            same_cluster = (i < N_coord//2) == (j < N_coord//2)
                            if not same_cluster:
                                thisa0 *= afac
                            if masses_copy[i] * masses_copy[j] > 0:
                                thisa0 *= samefac
                            ...
                            diq_i = 0 if i < 2 else (1 if i < 4 else 2)
                            diq_j = 0 if j < 2 else (1 if j < 4 else 2)
                            if diq_i != diq_j:
                                thisa0 *= afac
                            if masses_copy[i] * masses_copy[j] > 0:
                                thisa0 *= samefac
                        ...
                        if (k == i and l == j):
                            factor = ((1/(radial_n*thisa0)**2 - 2/(thisa0*rij))
                                      / masses[k]
                                      + (1/(radial_n*thisa0)**2 - 2/(thisa0*rij))
                                      / masses[l])
                            nabla_psi *= factor * hydrogen_wvfn(rij/thisa0,
                                                                radial_n)
                        else:
                            nabla_psi *= hydrogen_wvfn(rij/thisa0, radial_n)
        ...
        # ───── second: gradients hitting *different* factors ─────────
        for a in range(N_coord):
            for k in range(N_coord):
                for l in range(k, N_coord):
                    if k == l or (a != k and a != l):
                        continue
                    for m in range(N_coord):
                        for n in range(m, N_coord):
                            if (m == n or (m == k and n == l) or
                               (a != m and a != n)):
                                continue
                            for x in range(3):
                                ...
                                nabla_psi_tot += nabla_psi / abs(masses[a])
...
# ------------------------------------------------------------------
#  ▣  Pentaquark colour tensors   (N_coord == 5)
#      indices: 0 ≡ Q̄ , 1 ≡ Q   (meson) ,
#                2-3-4 ≡ Q Q Q  (baryon)
# ------------------------------------------------------------------
if N_coord == 5:

    # allocate colour bases, same shape as the master array
    S_1x1   = onp.zeros_like(S_av4p_metropolis)
    S_8x8   = onp.zeros_like(S_av4p_metropolis)
    S_8b8b  = onp.zeros_like(S_av4p_metropolis)

    # --- fundamental SU(3) generators  T^a = λ^a / 2 ---------------
    gell = onp.empty((8, 3, 3), dtype=complex)
    gell[0] = [[0, 1, 0], [1, 0, 0], [0, 0, 0]]               # λ1
    gell[1] = [[0, -1j, 0], [1j, 0, 0], [0, 0, 0]]            # λ2
    gell[2] = [[1, 0, 0], [0, -1, 0], [0, 0, 0]]              # λ3
    gell[3] = [[0, 0, 1], [0, 0, 0], [1, 0, 0]]               # λ4
    gell[4] = [[0, 0, -1j], [0, 0, 0], [1j, 0, 0]]            # λ5
    gell[5] = [[0, 0, 0], [0, 0, 1], [0, 1, 0]]               # λ6
    gell[6] = [[0, 0, 0], [0, 0, -1j], [0, 1j, 0]]            # λ7
    gell[7] = [[1/onp.sqrt(3), 0, 0],
               [0, 1/onp.sqrt(3), 0],
               [0, 0, -2/onp.sqrt(3)]]                        # λ8
    gell *= 0.5                                               # divide by 2

    # --- totally antisymmetric ε_{abc} -----------------------------
    eps = onp.zeros((3, 3, 3), dtype=int)
    for a, b, c in [(0, 1, 2), (1, 2, 0), (2, 0, 1)]:
        eps[a, b, c]  =  1
        eps[c, b, a]  = -1

    # normalisation factors so that ⟨T | T⟩ = 1 on every walker
    Ninv_1x1 = 1.0 / (3 * onp.sqrt(2))
    Ninv_8   = 0.5

    # ---------------------------------------------------------------
    for i in range(NI):            # colour of χ  (particle 0)
      for j in range(NI):          # colour of ψ  (particle 1)
        for k in range(NI):        #               (particle 2)
          for l in range(NI):      #               (particle 3)
            for m in range(NI):    #               (particle 4)

              spin_slice = (slice(None),) + (i,0, j,0, k,0, l,0, m,0)

              # ■ 1 ⊗ 1 : δ_{ij} ε_{klm}
              S_1x1[spin_slice] = Ninv_1x1 * (1 if i == j else 0) * eps[k, l, m]

              # ■ 8 ⊗ 8 : ½ T^a_{ij} T^a_{nk} ε_{nlm}
              sum8  = 0.0 + 0.0j
              for a in range(8):
                  for n in range(NI):
                      sum8 += gell[a, i, j] * gell[a, n, k] * eps[n, l, m]
              S_8x8[spin_slice] = Ninv_8 * sum8

              # ■ 8_b ⊗ 8_b : ½ T^a_{ik} T^a_{nm} ε_{njl}
              sum8b = 0.0 + 0.0j
              for a in range(8):
                  for n in range(NI):
                      sum8b += gell[a, i, k] * gell[a, n, m] * eps[n, j, l]
              S_8b8b[spin_slice] = Ninv_8 * sum8b

    # pick the requested colour
    if   color == "1x1":
        S_av4p_metropolis = S_1x1
    elif color == "8x8":
        S_av4p_metropolis = S_8x8
    elif color == "8b8b":
        S_av4p_metropolis = S_8b8b
    else:
        raise ValueError(f"Unsupported pentaquark colour state: {color}")

    # sanity-check normalisation on a single walker
    assert abs(adl.inner(S_av4p_metropolis, S_av4p_metropolis) - 1.0).max() < 1e-6
