...
parser.add_argument('--L_max', type=int, default=3) ################## NEW
parser.add_argument('--Rc', type=float, default=0.01)  ############################ it's more about 0.2
parser.add_argument('--verbose', dest='verbose', action='store_true', default=False)
globals().update(vars(parser.parse_args()))

#######################################################################################
###################################################################################################################
def VB_MRS_definition(alpha):    
    #######################################################################################
    ...
    #def Javad_symb_real(a, alpha):
    #    # Assuming Beta function is already compatible with JAX
    #    x = 1 / (2 * Beta(0) * alpha)
    #    term1 = np.exp(-x)
    #    # Using JAX's implementation for the lower incomplete gamma function
    #    # Note: gammainc in JAX is normalized by gamma(a), which differs from sympy's lowergamma
    #    term3 = jax.scipy.special.gammainc(-a, -x) * jax.scipy.special.gamma(-a)  # Adjusted for JAX; check this for correctness!
    #    term4 = (-x) ** a
    #    result = term1 * term3 * term4
    #    return np.real(result)  # Assuming result is complex; adjust if necessary
    
    #def Javad_symb_real(x, a):
    #    term1 = mpmath.exp(-x)
    ...
    def Javad_def(a, x):
        ...
        ## Numerically integrate the gamma function's integrand over [0, x] for lower incomplete gamma
        #def lower_gamma(a, x):
        #    # Define a JAX-compatible integration method here
        #    # This is a placeholder; actual JAX-based integration would be needed
        #    result, _ = quad(lambda t: gamma_integrand(t, a), 0, x)
        #    return result
        ...
        def trapezoidal_rule(f, a, b, N):
            ...
            Args:
            - f: the function to integrate, must be a JAX function.
            - a, b: the limits of integration.
            - N: the number of intervals to use in the approximation.

            Returns:
            - The approximate integral of f from a to b.
            """
            x = np.linspace(a, b, N)  # N+1 points make N intervals
            y = f(x)
            dx = (b - a) / N
            return (dx/2) * (y[0] + 2 * np.sum(y[1:N]) + y[N])
        
        def gamma_integrand(t, a):
            """
            The integrand for the lower incomplete gamma function.
            ...
            """
            # Ensure `a` can broadcast correctly with `t`
            # If `a` is scalar, this has no effect. If `a` is an array, make sure its shape is (1,) or compatible.
            #a_broadcasted = a * np.ones_like(t)
            return t**(a-1) * np.exp(-t)

        #@jit
        #def lower_gamma(a, x, N=99):
        #    """
        #    Approximate the lower incomplete gamma function using the trapezoidal rule.

        #    Args:
        #    - a: the shape aparameter of the gamma function.
        #    - x: the upper limit of integration.
        #    - N: the number of intervals for the trapezoidal rule approximation.

        #    Returns:
        #    - The approximate value of the lower incomplete gamma function.
        #    """
        #    f = lambda t: gamma_integrand(t, a)
        #    return trapezoidal_rule(f, 0, x, N)

        def lower_gamma(a, x, N=n_walkers):
            # Convert scalar `a` to an array with a single element if necessary
            a = np.atleast_1d(a)
            # Define a lambda function for the integrand parameterized by a
            f = lambda t, a: gamma_integrand(t, a)
            # Use vmap to vectorize trapezoidal_rule over elements of a
            integral_fn = vmap(lambda a: trapezoidal_rule(lambda t: f(t, a), 0, x, N), in_axes=(0,))
            result = integral_fn(a)
            return result if a.shape[0] > 1 else result[0]

        @jit
        def Javad_jax(a, x):
            term1 = np.exp(-x)
            term3 = lower_gamma(-a, -x)  # This needs a JAX-compatible integration method
            term4 = (-x)**a
            result = term1 * term3 * term4
            return np.real(result)  # Ensure result is real

        result = Javad_jax(a, x)
    ...
    #L_max = 3
    V0 = sum((k+1) * spl.gamma(1 + b) / spl.gamma(k + 2 + b) / (2 * Beta(0))**k * f(k) for k in range(0, L_max))
    ...
    Rl_list = [Rl(l) for l in range(0, L_max+1)]
    rl_list = [0.42, 1.03, 3.69, 17.4]

    def V_RS(alpha):
        result = sum((rl_list[i] - Rl_list[i]) * alpha**(i+1) for i in range(0, L_max))
        return result
    
    return V_RS(alpha) + V_Borel(alpha)
...
VB_LO = lambda R: VB_MRS_definition(Alpha_s1Loop(R))
...
VB_NLO = lambda R: VB_MRS_definition(Alpha_s2Loop(R))
VB_NNLO = lambda R: VB_MRS_definition(Alpha_s3Loop(R))
