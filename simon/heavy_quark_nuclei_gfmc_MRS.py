...
parser.add_argument('--Rc', type=float, default=1)  ############################ it's more about 0.2
...
ss6 = zeta51+zeta6 ############ different from Andreas' orginal code (mistake from Andreas I think)
...
def v(k):
    if k == 0:
        ...
        return a(2) + 2 * (2 * a(1) * Beta(0) + a(0) * Beta(1)) * L_fun(1) + a(0) * (2 * Beta(0))**2 * L_fun(2)
    elif k == 3:
...
#Have alpha bellow Rc fixed as allpha(Rc) ?
...
VB_LO = lambda R: VB_MRS_definition(Alpha_s1Loop(R), L_pert, 1) / (Nc - 1) ############################### NEW to be independent of the change in mu
...
VB_NLO = lambda R: VB_MRS_definition(Alpha_s2Loop(R), L_pert, 1) / (Nc - 1)
VB_NNLO = lambda R: VB_MRS_definition(Alpha_s3Loop(R), L_pert, 1) / (Nc - 1)
...
Rprime = lambda R: adl.norm_3vec(R)*np.exp(np.euler_gamma)*mu*s ######################### MODIFY
...
if OLO == "LO":
    ...
    def potential_fun(R):
        ...
        def true_fn():
            alpha_s2loop_Rc = Alpha_s2Loop(norm_Rc) #?
            potential = -1 * VB / norm_R * (1 + alpha / (4 * np.pi) * (2 * beta0 * np.log(Rprime(R)) + aa1))
            #potential = -1 * VB_NLO(norm_Rc) / norm_R * (1 + alpha_s2loop_Rc / (4 * np.pi) * (2 * beta0 * np.log(Rprime(R)) + aa1)) #?
            return potential
