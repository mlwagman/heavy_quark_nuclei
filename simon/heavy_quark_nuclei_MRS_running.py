...
def VB_MRS_definition(alpha):
    ...
    def V_RS(alpha):
        ...
        print('V_RS is ----------> ', result)#, result.shape)
        return result
    
    """
    the parameter GFMC tau=dtau weights gives only "nan + nan j" elements
    """
    
    
    #@partial(jax.jit)
    def Javad_jax_real(a, alpha): # <----------------------- HERE IS THE PROBLEM !!!!!
        print('alpah(R) type is ----------> ', type(alpha), alpha)
        x = 1/(2 * Beta(0) * alpha)
        #x = np.array(1/(2 * Beta(0) * alpha))
        ...
        def lower_incomplete_gamma(a_value, x_values):
            ...
            for i, x in enumerate(x_values):
                result = trapezoidal_rule(integrand, x, epsilon, N, a=a_value) #It dislike when x is close to 0 which is our case !
                """
                It dislike when x is close to 0 which is our case !!!!!!!
                x is might be between 1 and 10 which is consider close to 0.
                Because the good result are obtained when x is more like 400
                """
                #results[i] = result
        ...
        print('term3 is ', term3)
        
        term4 = (-x + 0j)**a
        print('term4 is ', term4)
        print('"a" parameter is ', a)
        result = term1 * term4
        #result = term1 * term3 * term4
        print('Javad is ----------> ', result, result.shape)
    ...
    def V_Borel(alpha):
        result = V0 * 1 / (2 * Beta(0)) * calc_Javad_symb_real(b, alpha)
        print('V_Borel is ----------> ', result)#, result.shape)
        return result
...
#CF = (Nc**2 - 1)/(2*Nc)
#VB = alpha*CF/(Nc-1)
VB = VB_MRS_definition(alpha) ######################## NEW
print('VB is ', VB)
SingC3 = -(Nc+1)/8
...
if OLO == "LO":
    ...
    def singlet_potential_fun(R):
        ...
        def false_fn():
            ...
            print("VB_NLO(norm_R) (singlet_potential_fun false_fn):", VB_NLO_val)  # Debugging shape
