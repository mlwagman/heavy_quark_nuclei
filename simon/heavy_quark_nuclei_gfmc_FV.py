...
dFA = Nc*(Nc**2+6)/48
alpha4 = float(mpmath.polylog(4,1/2))+(-np.log(2))**4/(4*3*2*1) ################ I'VE CHANGED THE "*0"
ss6 = zeta51+zeta6
...
#log_mu_r <=====> np.euleur_gamma ???????
VB_NNNLO = VB * (1 + alpha/(4*np.pi)*(aa1 + 2*beta0*log_mu_r) + (alpha/(4*np.pi))**2*( beta0**2*(4*log_mu_r**2 + np.pi**2/3) + 2*( beta1+2*beta0*aa1 )*log_mu_r + aa2 ) + (alpha/(4*np.pi))**3 * (aa3 + (8 log_mu_r**3 + 2 * log_mu_r * np.pi**2 + 16 * zeta3) * beta0**3 + 2 * log_mu_r * beta2 + ((12 * log_mu_r**2 + np.pi**2) * beta0**2 + 4 * log_mu_r * beta1) * aa1 + (6 * aa2 * log_mu_r + 5/2 * (4 * log_mu_r**2 + np.pi**2/3) * beta1) * beta0 + 16/3 * np.pi**2 * Nc**3 * np.log(Nc*alpha/2))################ NEW
#or
VB_NNNLO = VB * (1 + alpha/(4*np.pi)*(aa1 + 2*beta0*log_mu_r) + (alpha/(4*np.pi))**2*( beta0**2*(4*log_mu_r**2 + np.pi**2/3) + 2*( beta1+2*beta0*aa1 )*log_mu_r + aa2 ) + (alpha/(4*np.pi))**3 * (aa3 + (8 log_mu_r**3 + 2 * log_mu_r * np.pi**2 + 16 * zeta3) * beta0**3 + 2 * log_mu_r * beta2 + ((12 * log_mu_r**2 + np.pi**2) * beta0**2 + 4 * log_mu_r * beta1) * aa1 + (6 * aa2 * log_mu_r + 5/2 * (4 * log_mu_r**2 + np.pi**2/3) * beta1) * beta0 + 8 * np.pi**2 * Nc**3 * np.log(mu/q))#or 2*log_mu_r  instead of np.log(mu/q) ???
...
if OLO == "LO":
    ...
    def singlet_potential_fun_sum(R):
            return calculate_sum(potential_fun, R, L, nn)
    def octet_potential_fun(R):
        return spoilS*(Nc - 1)/CF/(2*Nc)*VB/adl.norm_3vec(R)*(1 + alpha/(4*np.pi)*(2*beta0*np.log(Rprime(R))+aa1) + (alpha/(4*np.pi))**2*( beta0**2*(4*np.log(Rprime(R))**2 + np.pi**2/3) + 2*( beta1+2*beta0*aa1 )*np.log(Rprime(R))+ aa2 + (Nc**2)*((np.pi)**4-12*(np.pi)**2) ) )# Why the last line ?? Color correction ?
    def octet_potential_fun_sum(R):
            return calculate_sum(symmetric_potential_fun, R, L, nn)
else:
