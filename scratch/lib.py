            ...
            for j in range(i+1, A):
                ...
                for name,op in two_body_ops.items():
                    ...
                    broadcast_src_snk_inds = (
                        ...
                        (np.newaxis,)*2*(A-j-1) # skip A-j-1 src iso/spin
                    )
...
def measure_gfmc_loss(
    ...
    def body(carry, array_elems):
        ...
        V_SI = V_SI[:,:1,:1,0,0]
        jax_print(np.sort(np.abs(V_SI[:,0,0,0,0]))[-10:], label='VSI', level=3)
        ...
        V_SI = V_SI[:,:1,:1,:1,:1,0,0,0,0]
        jax_print(np.sort(np.abs(V_SI[:,0,0,0,0]))[-10:], label='VSI', level=3)
        i = np.argmax(np.abs(V_SI[:,0,0,0,0]))
        jax_print((
        ...
        V_SI = V_SI[:,:1,:1,:1,:1,0,0,0,0]
        jax_print(np.sort(np.abs(V_SI[:,0,0,0,0]))[-10:], label='VSI 2', level=3)
        i = np.argmax(np.abs(V_SI[:,0,0,0,0]))
        jax_print((
