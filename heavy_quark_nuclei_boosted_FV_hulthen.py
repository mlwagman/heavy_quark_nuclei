...
def compute_Vtilde_G(G_values, delta, V0):
    # G_values: array of G magnitudes
    V_tilde = onp.zeros_like(G_values)

    for idx, G in enumerate(G_values):
        integrand = lambda r: (onp.exp(-delta * r) / (1 - onp.exp(-delta * r))) * r * onp.sin(G * r)
        integral, error = quad(integrand, 0, onp.inf, limit=1000)
        V_tilde[idx] = -V0 * (4 * onp.pi / G) * integral
...
def FV_Hulthen(R, L, nn):
    ...
    V0 = VB * delta  # VB and delta should be defined in your scope
    V_r = -V0 * np.exp(-delta * r_shifted_norm) / (1 - np.exp(-delta * r_shifted_norm))
    sums = np.sum(V_r, axis=1)  # Sum over periodic images
    return sums
...
# Define the screening parameter delta

if OLO == "LO":
    @partial(jax.jit)
    def singlet_potential_fun(R):
        r = adl.norm_3vec(R)
        V0 = VB * delta # Use the appropriate potential strength
        return -V0 * np.exp(-delta * r) / (1 - np.exp(-delta * r))
    @partial(jax.jit)
    def singlet_potential_fun_p(R):
        r = adl.norm_3vec(R)
        V0 = VB * delta / 2  # Adjust if necessary
        return V0 * np.exp(-delta * r) / (1 - np.exp(-delta * r))
    @partial(jax.jit)
...
AV_Hulthen = {}
B3_Hulthen = {}  # If you have any three-body terms

# For infinite volume
AV_Hulthen['OSing'] = singlet_potential_fun
AV_Hulthen['OSingp'] = singlet_potential_fun_p

# For finite volume, update if necessary
# AV_Hulthen['OSing'] = singlet_potential_fun_sum
# AV_Hulthen['OSingp'] = singlet_potential_fun_p_sum
...
if potential == "product":
    Hulthen_potential = adl.make_pairwise_product_potential(AV_Hulthen, B3_Hulthen, masses)
else:
    Hulthen_potential = adl.make_pairwise_potential(AV_Hulthen, B3_Hulthen, masses)
