...
if OLO == "LO":
    ...
    def symmetric_potential_fun(R):
            return spoilS*(- 1)*VB/adl.norm_3vec(R)
    @partial(jax.jit)
    def singlet_potential_fun(R):
            return -1*(- 1)*VB/adl.norm_3vec(R)
    @partial(jax.jit)
    def octet_potential_fun(R):
            return spoilS*VB/adl.norm_3vec(R)
    ...
    def symmetric_potential_fun_sum(R):
            return spoilS*(- 1)*VB*FV_Coulomb(R, L, nn)
    @partial(jax.jit)
    def singlet_potential_fun_sum(R):
            return -1*(- 1)*VB*FV_Coulomb(R, L, nn)
    @partial(jax.jit)
    def octet_potential_fun_sum(R):
            return spoilS*VB*FV_Coulomb(R, L, nn)
elif OLO == "NLO":
    @partial(jax.jit)
    def potential_fun(R):
    ...
    def symmetric_potential_fun(R):
        return (- 1)*VB*spoilS/adl.norm_3vec(R)*(1 + alpha/(4*np.pi)*(2*beta0*np.log(Rprime(R))+aa1))
    def symmetric_potential_fun_sum(R):
    ...
    def singlet_potential_fun_sum(R):
            return calculate_sum(potential_fun, R, L, nn)
    def octet_potential_fun(R):
        return spoilS*VB/adl.norm_3vec(R)*(1 + alpha/(4*np.pi)*(2*beta0*np.log(Rprime(R))+aa1))
    def octet_potential_fun_sum(R):
            return calculate_sum(symmetric_potential_fun, R, L, nn)
else:
