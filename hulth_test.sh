...
L=0
file="mtm_data_hulth/Hammys_LO_dtau${dt}_Nstep${nstep}_Nwalkers${nw}_Ncoord2_Nc1_nskip${nskip}_Nf0_alpha${alpha}_spoila1.0_log_mu_r0.0_wavefunction_compact_potential_full_afac1_masses[${m1}, -${m2}]_mtm[0, 0][0, 0][0, 0]_g0.0.h5"
echo "looking for $file"
if [ ! -f "$file" ]; then
$python heavy_quark_nuclei_boosted_FV_hulthen.py --alpha $alpha --log_mu_r 0.0 --mu $mu --OLO "LO" --n_step $nstep --n_walkers $nw --dtau $dt --Nc 1 --nf 0 --N_coord 2 --outdir "mtm_data_hulth/" --wavefunction $wvfn --potential "full" --Lcut 5 --L 0 --spoila $sa --n_skip $nskip --masses $m1 -$m2 --g $g --mtm_x 0 0 --mtm_y 0 0 --mtm_z 0 0 #--verbose
fi
fit="mtm_data_hulth/Hammys_LO_dtau${dt}_Nstep${nstep}_Nwalkers${nw}_Ncoord2_Nc1_nskip${nskip}_Nf0_alpha${alpha}_spoila1.0_log_mu_r0.0_wavefunction_compact_potential_full_afac1_masses[${m1}, -${m2}]_mtm[0, 0][0, 0][0, 0]_g0.0.csv"
if [ ! -f "$fit" ]; then
$python average_constant_fit.py "--database=${file}" --dtau $dt
fi

fit="mtm_data_hulth/Hammys_LO_dtau${dt}_Nstep${nstep}_Nwalkers${nw}_Ncoord2_Nc1_nskip${nskip}_Nf0_alpha${alpha}_spoila1.0_log_mu_r0.0_wavefunction_compact_potential_full_afac1_masses[${m1}, -${m2}]_mtm[0, 0][0, 0][0, 0]_g0.0_Lanczos.csv"
if [ ! -f "$fit" ]; then
$python Lanczos_gfmc.py "--database=${file}"  --n_skip=5 --n_block=10 --dtau $dt
fi

# Loop over the Lattice size
#for L in 1000 100 10 1; do
#for L in `seq 10 10 100`; do
for L in `seq 100 10 100`; do
...
#        python heavy_quark_nuclei_boosted_FV_hulthen.py --alpha $alpha --log_mu_r 0.0 --mu $mu --OLO "LO" --n_step $nstep --n_walkers $nw --dtau $dt --Nc 1 --nf 0 --N_coord 2 --outdir "mtm_data_qed/" --wavefunction $wvfn --potential "full" --Lcut 5 --L 0 --spoila $sa --n_skip $nskip --masses $m1 -$m2 --g $g --mtm_x $mtm_x_1 $mtm_x_2 --mtm_y $mtm_y_1 $mtm_y_2 --mtm_z $mtm_z_1 $mtm_z_2 --verbose
        file="mtm_data_hulth/Hammys_LO_dtau${dt}_Nstep${nstep}_Nwalkers${nw}_Ncoord2_Nc1_nskip${nskip}_Nf0_alpha${alpha}_spoila1.0_log_mu_r0.0_wavefunction_compact_potential_full_L${L}.0_afac1_masses[${m1}, -${m2}]_mtm[${mtm_x_1}, ${mtm_x_2}][${mtm_y_1}, ${mtm_y_2}][${mtm_z_1}, ${mtm_z_2}]_g0.0.h5"
        echo "looking for $file"
        if [ ! -f "$file" ]; then
        $python heavy_quark_nuclei_boosted_FV_hulthen.py --alpha $alpha --log_mu_r 0.0 --mu $mu --OLO "LO" --n_step $nstep --n_walkers $nw --dtau $dt --Nc 1 --nf 0 --N_coord 2 --outdir "mtm_data_hulth/" --wavefunction $wvfn --potential "full" --Lcut 5 --L $L --spoila $sa --n_skip $nskip --masses $m1 -$m2 --g $g --mtm_x $mtm_x_1 $mtm_x_2 --mtm_y $mtm_y_1 $mtm_y_2 --mtm_z $mtm_z_1 $mtm_z_2 #--verbose
        fi
        fit="mtm_data_hulth/Hammys_LO_dtau${dt}_Nstep${nstep}_Nwalkers${nw}_Ncoord2_Nc1_nskip${nskip}_Nf0_alpha${alpha}_spoila1.0_log_mu_r0.0_wavefunction_compact_potential_full_L${L}.0_afac1_masses[${m1}, -${m2}]_mtm[${mtm_x_1}, ${mtm_x_2}][${mtm_y_1}, ${mtm_y_2}][${mtm_z_1}, ${mtm_z_2}]_g0.0.csv"
        echo "looking for $fit"
        if [ ! -f "$fit" ]; then
        $python average_constant_fit.py "--database=${file}" --dtau $dt
        fi

        fit="mtm_data_hulth/Hammys_LO_dtau${dt}_Nstep${nstep}_Nwalkers${nw}_Ncoord2_Nc1_nskip${nskip}_Nf0_alpha${alpha}_spoila1.0_log_mu_r0.0_wavefunction_compact_potential_full_L${L}.0_afac1_masses[${m1}, -${m2}]_mtm[${mtm_x_1}, ${mtm_x_2}][${mtm_y_1}, ${mtm_y_2}][${mtm_z_1}, ${mtm_z_2}]_g0.0_Lanczos.csv"
        echo "looking for $fit"
        #if [ ! -f "$fit" ]; then
        $python Lanczos_gfmc.py "--database=${file}"  --n_skip=5 --n_block=10 --dtau $dt
        #fi
...
for file in mtm_data_hulth/Hammys*mtm*.h5
    do
