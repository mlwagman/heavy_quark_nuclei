...
# Operator momentum q
# Operator momentum q: same q for every particle, DO NOT divide by N_coord
if volume == "finite":
    q_mag = 2*np.pi/L
else:
    q_mag = Q
q_dir = n_mtm                      # shape (N_coord,3); typically one direction repeated
q_vec = q_mag * q_dir              # operator momentum (used ONLY in currents)
...
@partial(jax.jit)
def compute_phase_sum(Rs, q):
    # sum_k e^{i q·x_k} per walker
    dots = np.einsum('nai,ai->na', Rs, q)
    return np.sum(np.exp(1j * dots), axis=1)   # (Nw,)

@partial(jax.jit)
def _phase_per_particle(Rs, q):
    # (Nw, N_coord): per-particle phases e^{i q·x_k}
    return np.exp(1j * np.einsum('ai,nai->na', q, Rs))

@partial(jax.jit)
def _phase_sum(Rs, q):
    return np.sum(_phase_per_particle(Rs, q), axis=1)   # (Nw,)

@partial(jax.jit)
def compute_J_S(Rs, q):
    # Σ_k e^{iq·x_k} [ 1 + 1/4 ∇_k^2 + (i/4) q·∇_k ]  acting on phase-free ψ
    psi0 = f_R_0(Rs)                 # (Nw,)
    g0   = grad_f_R_0(Rs)            # (N_coord,3,Nw)
    lap0 = laplacian_f_R_0(Rs)       # (Nw,)
    ph_k = _phase_per_particle(Rs, q)  # (Nw,N_coord)

    S_base = np.sum(ph_k, axis=1)    # (Nw,)

    # per-particle q·∇_k ψ / ψ
    qdotg  = np.einsum('ain,ai->an', g0, q)     # (N_coord,Nw)
    grad_sum = np.sum(ph_k * (qdotg.T / psi0[None, :]), axis=1)  # (Nw,)

    return S_base + 0.25*(lap0/psi0)*S_base + 0.25j*grad_sum


@partial(jax.jit)
def compute_J_V_0(Rs, q):
    # Σ_k e^{iq·x_k} [ 1 - 1/4 ∇_k^2 - (i/4) q·∇_k ]  acting on phase-free ψ
    psi0 = f_R_0(Rs)
    g0   = grad_f_R_0(Rs)
    lap0 = laplacian_f_R_0(Rs)
    ph_k = _phase_per_particle(Rs, q)

    S_base = np.sum(ph_k, axis=1)
    qdotg  = np.einsum('ain,ai->an', g0, q)
    grad_sum = np.sum(ph_k * (qdotg.T / psi0[None, :]), axis=1)

    return S_base - 0.25*(lap0/psi0)*S_base - 0.25j*grad_sum


@partial(jax.jit)
def compute_J_V_k(Rs, q, i):
    # Σ_k e^{iq·x_k} [ i ∇_k^i - (1/2) q^i ]  acting on phase-free ψ
    psi0 = f_R_0(Rs)
    g0   = grad_f_R_0(Rs)                 # (N_coord,3,Nw)
    ph_k = _phase_per_particle(Rs, q)     # (Nw,N_coord)

    grad_piece = np.sum(ph_k * (g0[:, i, :].T / psi0[None, :]), axis=1)  # (Nw,)
    S_base     = np.sum(ph_k, axis=1)
    return 1j*grad_piece - 0.5*np.sum(q[:, i]) * S_base


@partial(jax.jit)
def compute_J_A_0(Rs, q):
    # axial charge = 0
    return np.zeros(Rs.shape[0], dtype=np.complex64)


@partial(jax.jit)
def compute_J_A_i(Rs, q, i):
    # Σ_k e^{iq·x_k} [ (1/(4 m^2)) (q × ∇_k)^i ] acting on phase-free ψ
    m = 1.0
    fac = 1.0/(4.0*m*m)
    ...
    total = np.zeros((Rs.shape[0],), dtype=np.complex64)
    for k in range(Rs.shape[1]):
        cross_q_grad = np.cross(q[k], g0[k], axis=0)  # (3,Nw)
        total += ph_k[:, k] * cross_q_grad[i]
    return fac * total


@partial(jax.jit)
def compute_J_T_0i(Rs, q, i):
    m = 1.0
    return (1.0/(2.0*m)) * q[0, i] * compute_phase_sum(Rs, q)


@partial(jax.jit)
def compute_J_T_i0(Rs, q, i):
    return -compute_J_T_0i(Rs, q, i)


@partial(jax.jit)
def compute_J_T_ij(Rs, q, i, j):
    # Σ_k e^{iq·x_k} [ (i/(4 m^2)) ( q^j ∇_k^i − q^i ∇_k^j ) ] acting on phase-free ψ
    m = 1.0
    fac  = 1.0/(4.0*m*m)
    g0   = grad_f_R_0(Rs)              # (N_coord,3,Nw)
    ph_k = _phase_per_particle(Rs, q)  # (Nw,N_coord)
    ...
    total = np.zeros((Rs.shape[0],), dtype=np.complex64)
    for k in range(Rs.shape[1]):
        term_k = (qj * g0[k, i] - qi * g0[k, j])  # (Nw,)
        total += ph_k[:, k] * 1j * term_k
    return fac * total
...
def J_Vi_ket0(Rs, q, i):
    psi0 = f_R_0(Rs)
    g0   = grad_f_R_0(Rs)
    ph_k = _phase_per_particle(Rs, q)
    grad_piece = np.sum(ph_k * (g0[:, i, :].T / psi0[None, :]), axis=1)
    S_base     = np.sum(ph_k, axis=1)
    return 1j*grad_piece - 0.5*np.sum(q[:, i]) * S_base
...
def J_Ai_ket0(Rs, q, i):
    ...
    total = np.zeros((Rs.shape[0],), dtype=np.complex64)
    for k in range(Rs.shape[1]):
        cross = np.cross(q[k], g0[k], axis=0)      # (3,Nw)
        total += ph_k[:, k] * cross[i]
    return fac * total

def J_T0i_ket0(Rs, q, i):
    m = 1.0
    return (1.0/(2.0*m)) * q[0, i] * compute_phase_sum(Rs, q)

def J_Ti0_ket0(Rs, q, i):
    return -J_T0i_ket0(Rs, q, i)

def J_Tij_ket0(Rs, q, i, j):
    m = 1.0
    fac = 1.0/(4.0*m*m)
    g0   = grad_f_R_0(Rs)
    ph_k = _phase_per_particle(Rs, q)
...
@partial(jax.jit, static_argnums=(1,))
def _psi_core_no_phase(Rs, wavefunction=bra_wavefunction, a0=a0, afac=afac, masses=absmasses):
...
def f_R(Rs, wavefunction=bra_wavefunction, a0=a0, afac=afac, masses=absmasses):
    psi = _psi_core_no_phase(Rs, wavefunction=wavefunction, a0=a0, afac=afac, masses=masses)
    phase = np.exp(1j*np.einsum('...ai,ai->...', Rs, mtm))  # uses your global `mtm` as before
    return psi * phase


def f_R_norm_0(Rs):
    # guide for mtm = 0 (strictly positive)
    return np.abs(_psi_core_no_phase(Rs))

def f_R_norm_q(Rs):
    # guide for mtm = q (keep it identical to f_R_norm_0 on purpose)
    return np.abs(_psi_core_no_phase(Rs))

def f_R_0(Rs):
    # physics ψ with mtm = 0
    return _psi_core_no_phase(Rs)

def f_R_q(Rs, q_vec):
    # physics ψ with mtm = q_vec (shape (N_coord,3))
...
def grad_core(Rs):
    # remove whatever global-phase grad_f_R attached
    return grad_f_R(Rs) / _phase(Rs, mtm)[None, None, :]

def lap_core(Rs):
    return laplacian_f_R(Rs) / _phase(Rs, mtm)


def grad_f_R_0(Rs):
    # gradient of phase-free ψ
    return grad_core(Rs)

def laplacian_f_R_0(Rs):
    # laplacian of phase-free ψ
    return lap_core(Rs)

def grad_f_R_q(Rs, q_vec):
    # ∇(ψ e^{iq·x}) = (∇ψ + i q ψ) e^{iq·x}
    g0  = grad_core(Rs)                            # (N_coord,3,Nw)
    psi = _psi_core_no_phase(Rs)                   # (Nw,)
    phase = _phase(Rs, q_vec)                      # (Nw,)
    # broadcast i*q*ψ onto (N_coord,3,Nw)
    iqpsi = 1j * q_vec[..., None] * psi[None, None, :]
    return (g0 + iqpsi) * phase[None, None, :]

def laplacian_f_R_q(Rs, q_vec):
    # ∇²(ψ e^{iq·x}) = (∇²ψ - q² ψ + 2 i q·∇ψ) e^{iq·x}
    lap0 = lap_core(Rs)                            # (Nw,)
    psi  = _psi_core_no_phase(Rs)                  # (Nw,)
    g0   = grad_core(Rs)                           # (N_coord,3,Nw)
    q2   = np.sum(q_vec**2)                        # scalar
    qdotg = np.einsum('ain,ai->n', g0, q_vec)      # (Nw,)
    phase = _phase(Rs, q_vec)                      # (Nw,)
    return (lap0 - q2*psi + 2j*qdotg) * phase
...
if n_step > 0:
    ...
    gfmc = adl.gfmc_deform(
        Rs_metropolis, S_av4p_metropolis, f_R_norm_0, params,
...
#TODO: save every 500 only do gfmc leg 2 steps of 100
...
# --- Operator phase sum with explicit q ---
phase_sums = []
for count, R in enumerate(gfmc_Rs):
    print('Calculating phase sum for step ', count)
    S_time = time.time()
    S = compute_phase_sum(R, q_vec)  # <--- pass q_vec, not global mtm
...
def J_S_ket0(Rs, q):
    ...
    g0   = grad_f_R_0(Rs)
    lap0 = laplacian_f_R_0(Rs)
    return _Ophase(Rs, q) * (1 + 0.25*lap0/psi0 + 0.25j*np.einsum('ain,ai->n', g0, q)/psi0)

def J_V0_ket0(Rs, q):
    psi0 = f_R_0(Rs)
    g0   = grad_f_R_0(Rs)
    lap0 = laplacian_f_R_0(Rs)
    # matches your structure but with ket=0 derivatives
    return _Ophase(Rs, q) * (1 - 0.25*lap0/psi0 - 0.25j*np.einsum('ain,ai->n', g0, q)/psi0)

def J_Vi_ket0(Rs, q, i):
    # i must be 0,1,2 (x,y,z)
    g0 = grad_f_R_0(Rs)
    base = _Ophase(Rs, q)
    # your earlier formula: e^{iq·x}( i ∑_k ∇_k^i − ½ ∑_k q_k^i )
    return base * (1j*np.sum(g0[:, i], axis=0) - 0.5*np.sum(q[:, i]))

def J_A0_ket0(Rs, q):
    # your code sets it to zero; keep that behavior
...
def J_Ai_ket0(Rs, q, i):
    ...
    fac = 1/(4*m*m)
    g0 = grad_f_R_0(Rs)  # (N_coord,3,Nw)
    phase_per_particle = np.exp(1j*np.einsum('ai,nai->na', q, Rs))
    total = 0
    for k in range(Rs.shape[1]):
        cross = np.cross(q[k], g0[k], axis=0)   # (3,Nw)
        total += phase_per_particle[:, k] * cross[i]
    return fac * total

def J_T0i_ket0(Rs, q, i):
    m = 1.0
    fac = 1/(2*m)
    base = compute_phase_sum(Rs, q)   # <--- sum over particles, not global phase
    return fac * q[0, i] * base
...
def J_Tij_ket0(Rs, q, i, j):
    ...
    total = 0
    for k in range(Rs.shape[1]):
        term_k = (q[0, j]*g0[k, i] - q[0, i]*g0[k, j])  # (Nw,)
        total += phase_per_particle[:, k] * 1j * term_k
    return fac * total



# --- All J with ket=0 derivatives and explicit q ---
J_S_sums   = []
J_V_0_sums = []
J_V_1_sums = []
J_V_2_sums = []
...
for count, R in enumerate(gfmc_Rs):
    ...
    S_time = time.time()
    J_S   = J_S_ket0(R, q_vec)
    J_V_0 = J_V0_ket0(R, q_vec)
    J_V_1 = J_Vi_ket0(R, q_vec, 0)  # x
    J_V_2 = J_Vi_ket0(R, q_vec, 1)  # y
    J_V_3 = J_Vi_ket0(R, q_vec, 2)  # z
    print(f"Calculated all J in {time.time() - S_time:.3f} sec")
    J_S_sums.append(J_S)
...
# Axial (arrays over τ1)
J_A_0_sums = []
J_A_1_sums = []
J_A_2_sums = []
J_A_3_sums = []
for count, R in enumerate(gfmc_Rs):
    J_A_0_sums.append(J_A0_ket0(R, q_vec))
    J_A_1_sums.append(J_Ai_ket0(R, q_vec, 0))
    J_A_2_sums.append(J_Ai_ket0(R, q_vec, 1))
    J_A_3_sums.append(J_Ai_ket0(R, q_vec, 2))
J_A_0_sums = np.array(J_A_0_sums)
J_A_1_sums = np.array(J_A_1_sums)
J_A_2_sums = np.array(J_A_2_sums)
J_A_3_sums = np.array(J_A_3_sums)

# Tensor (arrays over τ1)
J_T_0i_sums = []
J_T_i0_sums = []
J_T_ij_sums = {}
for count, R in enumerate(gfmc_Rs):
    J_T_0i_sums.append([J_T0i_ket0(R, q_vec, ix) for ix in range(3)])
    J_T_i0_sums.append([J_Ti0_ket0(R, q_vec, ix) for ix in range(3)])
    for ix in range(3):
        for jx in range(3):
            key = (ix, jx)
            val = J_Tij_ket0(R, q_vec, ix, jx)
            if key not in J_T_ij_sums: J_T_ij_sums[key] = []
...
# --- Three-point continuation for ALL currents ---
# For each τ1 (each step "count"), run a ψ_q-guided leg-2 once and reuse it
C3 = { 'S': [], 'V0': [], 'V1': [], 'V2': [], 'V3': [],
       'A0': [], 'A1': [], 'A2': [], 'A3': [],
       'T01': [], 'T02': [], 'T03': [], 'T10': [], 'T20': [], 'T30': [],
       # We'll store T_ij in a dict-of-lists; see below
     }
C3_Tij = { (i,j): [] for i in range(3) for j in range(3) }

# --- Two-point norms: C2_0(τ) from leg-1, C2_q(τ) from the leg-2 runs you already do ---
C2_0_tau = np.array([np.sum(W.real) for W in gfmc_Ws])  # shape (n_step+1,)
...
for count, R in enumerate(gfmc_Rs):
    ...
    # one guided evolution from R at t1
    rand2 = onp.random.random(size=(n_step, R.shape[0]))  # or choose n2 if you prefer
    gf2 = adl.gfmc_deform(
        R, gfmc_Ss[count], f_R_norm_q, params,
        rand_draws=rand2, tau_iMev=tau_iMev, N=n_step, potential=Coulomb_potential,
        deform_f=deform_f, m_Mev=np.abs(np.array(masses)), resampling_freq=resampling
    )
    W2_final = np.array([Ws for _,_,_,Ws in gf2])[-1]  # (Nw,)
    C2_q_tau_list.append(np.sum(W2_final.real))


    # ratio ψ_q/ψ_0 at the insertion configs
    ratio_0q = f_R_q(R, q_vec) / f_R_0(R)

    # compute ALL J insertions at τ1 (ket=0 versions)
    J_S   = J_S_sums[count]
    J_V0  = J_V_0_sums[count]
    J_V1  = J_V_1_sums[count]
    J_V2  = J_V_2_sums[count]
    J_V3  = J_V_3_sums[count]
    J_A0  = J_A0_ket0(R, q_vec)
    J_A1  = J_Ai_ket0(R, q_vec, 0)
    J_A2  = J_Ai_ket0(R, q_vec, 1)
    J_A3  = J_Ai_ket0(R, q_vec, 2)
    JT01  = J_T0i_ket0(R, q_vec, 0); JT02 = J_T0i_ket0(R, q_vec, 1); JT03 = J_T0i_ket0(R, q_vec, 2)
    JT10  = J_Ti0_ket0(R, q_vec, 0); JT20 = J_Ti0_ket0(R, q_vec, 1); JT30 = J_Ti0_ket0(R, q_vec, 2)

    # tensor ij
    JTij = { (i,j): J_Tij_ket0(R, q_vec, i, j) for i in range(3) for j in range(3) }

    # combine (leg-1) * (insertion) * (rebasing) * (leg-2)
    def combine(Jloc):
        return np.sum((W1 * Jloc * ratio_0q * W2_final).real)

    C3['S' ].append(combine(J_S))
    C3['V0'].append(combine(J_V0))
    C3['V1'].append(combine(J_V1))
    C3['V2'].append(combine(J_V2))
    C3['V3'].append(combine(J_V3))
    C3['A0'].append(combine(J_A0))
    C3['A1'].append(combine(J_A1))
    C3['A2'].append(combine(J_A2))
    C3['A3'].append(combine(J_A3))
    C3['T01'].append(combine(JT01)); C3['T02'].append(combine(JT02)); C3['T03'].append(combine(JT03))
    C3['T10'].append(combine(JT10)); C3['T20'].append(combine(JT20)); C3['T30'].append(combine(JT30))
    for ij in JTij:
        C3_Tij[ij].append(combine(JTij[ij]))


C2_q_tau = np.array(C2_q_tau_list)

# Normalized ratios that plateau (what you typically fit)
Ratio = {k: C3[k] / np.sqrt(C2_0_tau * C2_q_tau) for k in C3}        # S, V0..V3, A0..A3, T01..T30
Ratio_Tij = {ij: C3_Tij[ij] / np.sqrt(C2_0_tau * C2_q_tau) for ij in C3_Tij}

# arrays over τ1
for k in C3:      C3[k]      = np.array(C3[k])
for ij in C3_Tij: C3_Tij[ij] = np.array(C3_Tij[ij])
...
with h5py.File(filename, 'a') as f:
    f.create_dataset("C2_0_tau", data=C2_0_tau)
    f.create_dataset("C2_q_tau", data=C2_q_tau)
    for k, arr in C3.items():
        f.create_dataset(f"C3_{k}_tau1", data=arr)
    for (i,j), arr in C3_Tij.items():
        f.create_dataset(f"C3_T{i}{j}_tau1", data=arr)
    for k, arr in Ratio.items():
        f.create_dataset(f"Ratio_{k}_tau1", data=arr)
    for (i,j), arr in Ratio_Tij.items():
        f.create_dataset(f"Ratio_T{i}{j}_tau1", data=arr)
...
if verbose:
    ...
    ave_H  = np.array([al.bootstrap((K+V).real, W, Nboot=100, f=adl.rw_mean)
                    for K, V, W in zip(Ks, Vs, gfmc_Ws)])
    ave_Ks = np.array([al.bootstrap(K.real,      W, Nboot=100, f=adl.rw_mean)
                    for K, V, W in zip(Ks, Vs, gfmc_Ws)])
    ave_Vs = np.array([al.bootstrap(V.real,      W, Nboot=100, f=adl.rw_mean)
                    for K, V, W in zip(Ks, Vs, gfmc_Ws)])

    ave_phase = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(phase_sums, gfmc_Ws)])
    ave_J_S   = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_S_sums,   gfmc_Ws)])
    ave_J_V_0 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_V_0_sums, gfmc_Ws)])
    ave_J_V_1 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_V_1_sums, gfmc_Ws)])
    ave_J_V_2 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_V_2_sums, gfmc_Ws)])
    ave_J_V_3 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_V_3_sums, gfmc_Ws)])

    ave_J_A_0 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_A_0_sums, gfmc_Ws)])
    ave_J_A_1 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_A_1_sums, gfmc_Ws)])
    ave_J_A_2 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_A_2_sums, gfmc_Ws)])
    ave_J_A_3 = np.array([al.bootstrap(S.real, W, Nboot=100, f=adl.rw_mean)
                        for S, W in zip(J_A_3_sums, gfmc_Ws)])

    # Tensor: fix off-by-one and include i=0,1,2
    ave_J_T_0i = []
    for i in range(3):
        comp_i = np.array([X[i] for X in J_T_0i_sums])  # shape (Nτ, Nw)
        ave_J_T_0i.append(np.array([
            al.bootstrap(x.real, W, Nboot=100, f=adl.rw_mean)
            for x, W in zip(comp_i, gfmc_Ws)
        ]))
    ave_J_T_0i = np.array(ave_J_T_0i)  # (3, Nτ)

    ave_J_T_i0 = []
    for i in range(3):
        comp_i = np.array([X[i] for X in J_T_i0_sums])
        ave_J_T_i0.append(np.array([
            al.bootstrap(x.real, W, Nboot=100, f=adl.rw_mean)
            for x, W in zip(comp_i, gfmc_Ws)
        ]))
    ave_J_T_i0 = np.array(ave_J_T_i0)  # (3, Nτ)

    ave_J_T_ij = {}
    for (i, j), arr in J_T_ij_sums.items():  # arr shape: (Nτ, Nw)
        ave_J_T_ij[(i, j)] = np.array([
            al.bootstrap(x.real, W, Nboot=100, f=adl.rw_mean)
            for x, W in zip(arr, gfmc_Ws)
        ])

    with h5py.File(outdir + 'averages_' + tag + '.h5', 'w') as f:
        f['H']      = ave_H
        f['K']      = ave_Ks
        f['V']      = ave_Vs
        f['phase']  = ave_phase
        ...
        f['J_A_2']  = ave_J_A_2
        f['J_A_3']  = ave_J_A_3
        f['J_T_0i'] = ave_J_T_0i   # (3, Nτ)
        f['J_T_i0'] = ave_J_T_i0   # (3, Nτ)
        for (i, j), arr in ave_J_T_ij.items():
            f[f'J_T_{i}{j}'] = arr  # (Nτ,)
