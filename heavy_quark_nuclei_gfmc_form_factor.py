...
for count, R in enumerate(gfmc_Rs):
    ...
    phase = np.exp(1j*np.einsum('nai,ai->na', Rs, q_mtm))

    grad = grad_f_R(R)

    q_dot_grad = np.einsum('ain,ai->na', grad, q_mtm)

    J_term = np.sum( phase * q_dot_grad, axis=1 ) / f_R(R, wavefunction=bra_wavefunction)

    Js.append(J_term)

    print(f"calculated kinetic in {time.time() - J_time} sec")
Js = np.array(Js)
