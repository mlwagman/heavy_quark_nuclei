...
nn = np.array([np.array([i, j, k]) for i in range(-Lcut, Lcut+1) for j in range(-Lcut, Lcut+1) for k in range(-Lcut, Lcut+1)])

def calculate_sum(potential_func, R, L, nn):
    sums = np.zeros_like(potential_func(R))
    for n in nn:
        sums += potential_func(R - n * L)
    return sums
